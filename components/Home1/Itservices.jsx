      ...
      <p className='bg-black/10 text-lg font-semibold w-fit rounded-md px-4 py-2 text-violet-900'>
        ...
        <div className='mt-8 grid grid-cols-2 gap-12'>
          {itservices.map((service) => (
             <article key={service.id} className='my-2 group col-span-2 md:col-span-1'>
               <Link href="/" className='text-4xl lg:text-5xl font-bold text-gray-800 group-hover:text-violet-700'>
                {service.title}
               </Link>
               <p className='text-lg sm:font-semibold text-gray-500 my-5'>{service.text}</p>
