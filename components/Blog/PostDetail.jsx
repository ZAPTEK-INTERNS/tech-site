 ...
 const postId = blogsData[0].id
  ...
  return (
    <section className="mx-[7%] w-[86%] sm:mx-[5%] sm:w-[90%] mb-[60px]">
      ...
      <Image src={blogsData[0].image} alt={blogsData[0].keyword} className='rounded-md'/>
      <h1 className='my-6 pl-3 border-l-4 border-violet-700 text-xl sm:text-2xl md:text-3xl font-bold text-gray-800'>
        {blogsData[0].quote}
      </h1>
      <p className='text-lg text-gray-600 font-semibold'>{blogsData[0].introduction}</p>
      <div className='my-8'>
        <h1 className='text-3xl sm:text-4xl md:text-5xl mb-4 font-bold text-gray-800'>
          Common cybersecurity threats:
        </h1>
        <p className='text-gray-600 text-lg font-semibold'>{blogsData[0].threat}</p>
      </div>
      <div className='my-6'>
