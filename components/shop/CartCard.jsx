...
const CartCard = ({ ProductLink, CategoryLink, CartImg, CartTag, CartTitle, CartPrice, CartKey, }) => {
  return (
    <>
    <div className='space-y-8' key={CartKey}>
        <div className="relative">
            <Link href={`/product/` + ProductLink} className='overflow-hidden'><Image src={CartImg} className='transition-transform duration-150 ease-linear rounded hover:scale-105' /></Link>
        ...
        <div className="flex flex-row items-start justify-between">
            ...
            <h2 className="text-2xl font-bold transition-colors duration-300 ease-linear md:text-2xl hover:text-purple-800"><Link href={`/product/` + ProductLink}>{CartTitle}</Link></h2>
